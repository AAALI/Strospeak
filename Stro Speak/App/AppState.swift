...
import Carbon
import os.log
private let recordingLog = OSLog(subsystem: "siro.company.strospeak", category: "Recording")
...
enum AppBuild {
    static var isDevBundle: Bool {
        (Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String) == "Stro Speak Dev"
    }
}
...
final class AppState: ObservableObject, @unchecked Sendable {
    ...
    /// URL of the flag file written while Stro Speak is actively recording.
    ///
    ...
    ///
    /// Path: `~/Library/Application Support/Stro Speak/is-recording`
    /// (or `Stro Speak Dev/is-recording` when running the dev bundle).
    static func recordingStateFlagURL() -> URL {
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        let appName = Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String ?? "Stro Speak"
    ...
    private static let recordingStateFlagQueue = DispatchQueue(
        label: "siro.company.strospeak.recording-state-flag"
    )
    ...
    private func beginCriticalDictationActivity() {
        ...
        ProcessInfo.processInfo.disableAutomaticTermination("Stro Speak dictation in progress")
        automaticTerminationDisabled = true
    }

    private func endCriticalDictationActivity() {
        guard automaticTerminationDisabled else { return }
        ProcessInfo.processInfo.enableAutomaticTermination("Stro Speak dictation in progress")
