...
final class AudioRecorder: NSObject, ObservableObject, AVCaptureAudioDataOutputSampleBufferDelegate {
    ...
    private let sessionQueue = DispatchQueue(label: "siro.company.strospeak.capture.session")
    private let sampleBufferQueue = DispatchQueue(label: "siro.company.strospeak.capture.samples")
